...
class TouristicServiceTest {
    ...
    @Test
    void testGetAll() {
        when(touristicPlaceRepository.getAll()).thenReturn(Arrays.<TouristicPlace>asList(new TouristicPlace(Long.valueOf(1), "name", "type", "address", "contactInfo")));

        List<TouristicPlace> result = touristicService.getAll();
        Assertions.assertEquals(Arrays.<TouristicPlace>asList(new TouristicPlace(Long.valueOf(1), "name", "type", "address", "contactInfo")), result);
    }
    ...
    @Test
    void testUpdateTouristicPlaceAddress() {
        when(touristicPlaceRepository.findById(anyLong(), any())).thenReturn(new TouristicPlace(Long.valueOf(1), "name", "type", "address", "contactInfo"));

        touristicService.updateTouristicPlaceAddress(Long.valueOf(1), "address");
    }
